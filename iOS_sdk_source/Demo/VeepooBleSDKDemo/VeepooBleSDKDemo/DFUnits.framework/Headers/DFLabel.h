...
/** An enum that defines the types of `DFLabel` scrolling */
typedef NS_ENUM(NSUInteger, DFLabelType) {
...
/**
 DFLabel is a UILabel subclass adds a scrolling DFLabel effect when the text of a label instance outgrows the available width. Instances of `DFLabel` can be configured
...
////////////////////////////////////////////////////////////////////////////////
/// @name Creating DFLabels
////////////////////////////////////////////////////////////////////////////////

/** Returns a newly initialized `DFLabel` instance.
...
/** Returns a newly initialized `DFLabel` instance with the specified scroll rate and edge transparency fade length.
...
/** Returns a newly initialized `DFLabel` instance with the specified scroll duration and edge transparency fade length.
...
/** A boolean property that sets whether the `DFLabel` should behave like a normal UILabel.
 
 When set to `YES` the `DFLabel` will behave like a normal UILabel, and  will not begin scrolling when the text is
 larger than the specified frame. The change takes effect immediately, removing any in-flight animation as well as any
 current edge fade. Note that the `DFLabel` will respect the current values of the `lineBreakMode` and `textAlignment`
...
/** A boolean property that sets whether the `DFLabel` should hold (prevent) label scrolling.
 
 When set to `YES`, the `DFLabel` will not automatically scroll even its text is larger than the specified frame,
 although the specified edge fades will remain.
 
 To set the `DFLabel` to act like a normal UILabel, use the `labelize` property.
...
/** A boolean property that sets whether the `DFLabel` should only begin a scroll when tapped.
 
 If this property is set to `YES`, the `DFLabel` will begin a scroll animation cycle only when tapped. The label will
...
/** Defines the direction and method in which the `DFLabel` instance scrolls.
 
 `DFLabel` supports four types of scrolling: `DFLeftRight`, `DFRightLeft`, `DFContinuous`, and `DFContinuousReverse`.
 
 Given the nature of how text direction works, the options for the `DFLabelType` property require specific text alignments
 and will set the textAlignment property accordingly.
 
 - `DFLeftRight` type is ONLY compatible with a label text alignment of `NSTextAlignmentLeft`.
 - `DFRightLeft` type is ONLY compatible with a label text alignment of `NSTextAlignmentRight`.
 - `DFContinuous` does not require a text alignment (it is effectively centered).
 - `DFContinuousReverse` does not require a text alignment (it is effectively centered).
 
 Defaults to `DFLeftRight`.
...
@property (nonatomic, assign) IBInspectable NSInteger labelType;
#else
@property (nonatomic, assign) DFLabelType labelType;
#endif
...
/** Defines the duration of the scrolling animation.
 ...
 scrolling cycle. Note that for `DFLeftRight` and `DFRightLeft`, a cycle consists of the animation away,
...
/** A buffer (offset) between the leading edge of the label text and the label frame.
 ...
 @note For `DFContinuous`-type labels, the smallest value of `leadingBuffer`, 'trailingBuffer`, and `fadeLength`
...
@property (nonatomic, assign) CGFloat continuousDFLabelExtraBuffer DEPRECATED_ATTRIBUTE;


/** The length of transparency fade at the left and right edges of the `DFLabel` instance's frame.
 
 This propery sets the size (in points) of the view edge transparency fades on the left and right edges of a `DFLabel`. The
...
/** Convenience method to restart all `DFLabel` instances that have the specified view controller in their next responder chain.
 
 This method sends a `NSNotification` to all `DFLabel` instances with the specified view controller in their next responder chain.
 ...
 @warning View controllers that appear with animation (such as from underneath a modal-style controller) can cause some `DFLabel` text
...
/** Labelizes all `DFLabel` instances that have the specified view controller in their next responder chain.
...
/** De-Labelizes all `DFLabel` instances that have the specified view controller in their next responder chain.
